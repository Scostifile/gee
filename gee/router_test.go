...
func TestGetRoute(t *testing.T) {
	r := newTestRouter()
	n, ps := r.getRoute("GET", "/hello/timo")
	...
	if ps["name"] != "timo" {
		t.Fatal("name should be equal to 'timo'")
	}
...
func TestGetRoute2(t *testing.T) {
	...
	ok1 := n1.pattern == "/assets/*filepath" &&
		ps1["filepath"] == "file1.txt"
	if !ok1 {
		t.Fatal("pattern should be '/assets/*filepath' & filepath should be 'file1.txt'")
	}

	n2, ps2 := r.getRoute("GET", "/assets/css/test.css")
	ok2 := n2.pattern == "/assets/*filepath" &&
		ps2["filepath"] == "css/test.css"
	if !ok2 {
